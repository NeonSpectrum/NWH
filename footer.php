	...
	<?php
		...
		foreach (glob(__DIR__."/js/required/*.js") as $js) {
			$file = str_replace(__DIR__, "", $js);
			echo "<script src='$file?v=" . filemtime($js) . "'></script>\n";
		}
		if (strpos($_SERVER['PHP_SELF'],"admin")) {
			echo "<script src='/js/admin.js?v=".filemtime(__DIR__."/js/admin.js")."'></script>\n";
		} else {
			echo "<script src='/js/main.js?v=".filemtime(__DIR__."/js/main.js")."'></script>\n";
		}
	
		if (file_exists(__DIR__."/js/$currentDirectory.js") && $currentDirectory != 'admin') {
			echo "<script src='/js/$currentDirectory.js?v=".filemtime(__DIR__."/js/$currentDirectory.js")."'></script>\n";
		}
		if ((isset($_SESSION['accountType']) && $_SESSION['accountType']!="Owner") || isset($_SESSION) || $_SERVER['SERVER_NAME'] != "localhost") {
			echo "<script src='/js/verifyLoginSession.js?v=" . filemtime(__DIR__."/js/verifyLoginSession.js") . "'></script>\n";
		}
	?>
