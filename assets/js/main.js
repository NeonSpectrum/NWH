...
$(document).ready(function() {
  ...
  $('.modal').on('shown.bs.modal', function() {
    if (
      $(this)
        .find('form')
        .find('input.checkDate').length > 0
    ) {
      tempCheckDate = $(this)
        .find('form')
        .find('input.checkDate')
        .val()
    }
  })
...
Pace.on('done', function() {
  ...
  $('#pace').attr(
    ...
    $('#pace')
      .attr('href')
      .replace('pace-theme-center-simple', 'pace-theme-minimal')
  )
  // BACK TO TOP
  ...
  $('.frmBookCheck')
    ...
    .on('showCalendar.daterangepicker', function(ev, picker) {
      if (
        picker.element.offset().top - $(window).scrollTop() + picker.container.outerHeight() > $(window).height() &&
  ...
  $('input.checkDate').on('apply.daterangepicker', function(ev, picker) {
    if (picker.startDate.format('MM/DD/YYYY') == picker.endDate.format('MM/DD/YYYY')) {
      var nextDay = moment(picker.startDate)
        .add(1, 'days')
        .format('MM/DD/YYYY')
    ...
    $('html, body').animate(
      {
        scrollTop:
          screen.width > 480
            ? $(window.location.hash).offset().top - 80
            : $(window.location.hash).offset().top - 10 + 'px'
      },
      1000,
      'swing'
    )
  ...
  $('#modalEditReservation')
    ...
    .change(function() {
      ...
      $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        data: 'cmbBookingID=' + $(this).val() + '&root=' + root,
        success: function(response) {
          $('#modalEditReservation')
            .find('#txtCheckDate')
            .val(response[0])
          $('#modalEditReservation')
            .find('#txtAdults')
            .val(parseInt(response[1]))
          $('#modalEditReservation')
            .find('#txtChildren')
            .val(parseInt(response[2]))
          $('#modalEditReservation')
            .find('#txtPaymentMethod')
            .val(response[3])
  ...
  var currentDate = $('#modalEditReservation')
    .find('#txtCheckDate')
    .val()
  var currentRoomQuantity = []
  $('#modalEditReservation')
    .find('label.lblRoomType')
    .each(function() {
      currentRoomQuantity.push(
        $(this)
          .parent()
          .find('.cmbQuantity')
          .html()
          ...
          success: function(response) {
            if (response != false) {
              ...
              $(this)
                ...
                .show(function() {
                  $(this).html(
                    '<div class="alert alert-danger animated bounceIn"> <span class="glyphicon glyphicon-info-sign"></span> &nbsp; Already Paid!</div>'
                  )
                })
  ...
  $('#frmLogin').submit(function(e) {
    ...
    $(this)
      .find('#btnLogin')
      .html('<i class="fa fa-spinner fa-pulse"></i> Signing In ...')
    $(this)
      .find('#btnLogin')
      .attr('disabled', true)
    $.ajax({
  ...
  $('#frmRegister')
    ...
    .submit(function(e) {
      ...
      $(this)
        .find('#btnRegister')
        .html('<i class="fa fa-spinner fa-pulse"></i> Submitting...')
      $(this)
        .find('#btnRegister')
        .attr('disabled', true)
      $(this)
  ...
  $('#frmForgot').submit(function(e) {
    ...
    $(this)
      .find('#btnReset')
      .html('<i class="fa fa-spinner fa-pulse"></i> Sending ...')
    $(this)
      .find('#btnReset')
      .prop('disabled', true)
    $(this)
            ...
            socket.emit('notification', {
              ...
              messages:
                ...
                $(this)
                  .find('#cmbBookingID option:selected')
                  .html() +
                '</a> added a Bank Reference Picture'
            })
          }
        } else {
          $(this)
  ...
  $('.frmBookCheck').submit(function(e) {
    ...
    $(this)
      .find('#btnBookNow')
      .html('<i class="fa fa-spinner fa-pulse"></i> Booking...')
    $(this)
      .find('#btnBookNow')
      .prop('disabled', true)
    $(this).submit()
  ...
  $(':file').change(function() {
    if (this.files && this.files[0] && this.files[0].name.match(/\.(jpg|jpeg|png|gif)$/i)) {
      if (this.files[0].size > 2097152) {
        swal('Error!', 'File size is larger than 2MB!', 'error')
        $(this).val('')
      }
    } else {
      swal('Error!', 'This is not an image file!', 'error')
      $(this).val('')
    }
  })
})
...
function generateRoomID() {
  var roomType = $('#frmEditReservation')
    .find('#cmbRoomType')
    .val()
  var checkDate = $('#frmEditReservation')
    .find('#txtCheckDate')
    .val()
  var roomList = []
  $.ajax({
