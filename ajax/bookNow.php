...
if ($_SERVER['REQUEST_METHOD'] == "POST" && $system->validateToken($data['csrf_token'])) {
  ...
  if ($db->affected_rows > 0) {
    ...
    if ($arr[0] != false) {
      ...
      $arr[2]    = "http://{$_SERVER['SERVER_NAME']}{$root}payment/process.php/?" . $system->encrypt("txtBookingID=$bookingID&txtAmount=$totalRoomPrice&rooms=$roomsJson&csrf_token={$data['csrf_token']}");
